...
while ($true) {
    ...
    $claudeJob = Start-Job -ScriptBlock {
        ...
        docker exec -i -w $repoPath $containerName bash -c 'DOCKER_GID=$(stat -c "%g" /var/run/docker.sock 2>/dev/null || echo ""); if [ -n "$DOCKER_GID" ]; then sg $DOCKER_GID -c "claude --dangerously-skip-permissions /continue-work"; else claude --dangerously-skip-permissions /continue-work; fi'
